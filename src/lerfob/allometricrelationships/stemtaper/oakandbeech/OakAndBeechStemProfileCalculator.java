...
import lerfob.allometricrelationships.stemtaper.oakandbeech.OakAndBeechStemTaperTree.Species;
...
public class OakAndBeechStemProfileCalculator extends StemTaperModel {
	...
	@Override
	public StemTaperEstimate getPredictedTaperForTheseHeights(BasicStemTaperTree tree, List<Double> heightMeasures) {
		...
		StemTaperEstimate estimate = new OakAndBeechStemTaperEstimate(heightMeasures);
	...
	protected List<Double> getHeightsArrayWithSmallEndDiameter(OakAndBeechStemTaperTree tree, double initialHeightM, double smallEndCm) {
		...
		StemTaperEstimate taperEstimate = getPredictedTaperForTheseHeights(tree, heightMeasures);
		...
		int i = 0;
		for (i = 0; i < 100; i++) {
			diameter0 = taperEstimate.getMean().m_afData[i][0];
			diameter1 = taperEstimate.getMean().m_afData[i+1][0];
