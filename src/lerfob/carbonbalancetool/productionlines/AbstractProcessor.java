	...
	public Collection<ProcessUnit> doProcess(List<ProcessUnit> inputUnits) {
		for (ProcessUnit processUnit : inputUnits) {
			...
			if (biomass != null && functionUnitBiomass > 0) {
				double fonctionalUnits = biomass / (functionUnitBiomass * 1000d);		// 1000 to report it in tons and not in kg 
				double emissions = fonctionalUnits * emissionsByFunctionalUnit * 1000d;	// 1000 to report it in tons and not in kg
