...
public class MathildeThinningPredictorTest {
		...
		if (Trees == null) {
			...
			try {
				...
				while((record = reader.nextRecord()) != null) {
					...
					MathildeThinningTreeImpl tree = new MathildeThinningTreeImpl(treeId, dbhCm, speciesCode, linearTreePred, pred, stand);
					Trees.add(tree);
				}
			} catch (IOException e) {
	...
	public void TreePredictionsFixedEffectsOnlyTest() {
		...
		for (MathildeThinningTreeImpl tree : Trees) {
			...
			Assert.assertEquals(tree.getLinearTreePred(), actualLinearPred, 1E-8);
			nbTested++;
		}
