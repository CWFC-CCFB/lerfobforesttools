...
public final class MathildeTreeLoggerParameters extends TreeLoggerParameters<MathildeTreeLogCategory> {
	...
	protected void initializeDefaultLogCategories() {
		...
		for (MathildeTreeSpecies species : MathildeTreeSpecies.values()) {
			...
			categories.add(new MathildeTreeLogCategory(species, Grade.LargeLumberWood.name(), Grade.LargeLumberWoodExplanations, largeLumberWoodLimit, 0.84 * .35));
			categories.add(new MathildeTreeLogCategory(species, Grade.SmallLumberWood.name(), Grade.SmallLumberWoodExplanations, 27.5, 0.50 * .35));
